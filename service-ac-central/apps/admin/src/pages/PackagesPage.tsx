...
export default function PackagesPage() {
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) =>
        matchesQuery([item.name, item.pkSize], normalizedQuery),
      ),
  ...
  function addChecklistItem() {
    ...
    const next: ChecklistItem[] = [
      ...(form.checklist ?? []),
      { item: checklistDraft.trim(), included: true },
    ];
  ...
  function toggleChecklistItem(index: number) {
    const next = (form.checklist ?? []).map((c, i) =>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {filteredItems.map((pkg) => (
          <div key={pkg._id} className="clay-card">
            ...
            <ul className="mt-3 space-y-1 text-xs text-ink-500">
              ...
              {pkg.checklist.length > 3 && (
                <li>+{pkg.checklist.length - 3} item lainnya</li>
              )}
            </ul>
          ...
          <FormField label="Checklist Item Pekerjaan">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="clay-btn clay-btn-dark"
              >
                Tambah
              </button>
            </div>
            <div className="mt-3 space-y-2">
              {(form.checklist ?? []).map((c, i) => (
                <div
                  key={i}
                  className="clay-sm flex items-center gap-2 px-3 py-2"
                >
                  <input
                    type="checkbox"
                    checked={c.included}
                  ...
                  />
                  <span
                    className={`flex-1 text-sm ${c.included ? "text-ink-700" : "text-ink-400 line-through"}`}
                  >
                    {c.item}
                  </span>
                  <button
